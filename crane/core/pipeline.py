...
from ..step1 import InputContract, PreprocessOptions, SamplingOptions, Step1Options, prepare_input, run_step1
from .bridge import Step2BridgeOptions, build_step2_public_outputs, run_step2_from_step1
...
def run_pipeline(
    ...
    """Run the default CRANE Step 1 and Step 2 pipeline."""
    ...
    prepared_input = prepare_input(
        ...
        options=PreprocessOptions(
            batch_key=internal.get("batch_key"),
    ...
    step1_result = run_step1(
        ...
        step1_options=Step1Options(
            ...
            init_ratio=internal.get("init_ratio"),
            init_ratio_case_cap=internal.get("init_ratio_case_cap", 320),
            init_ratio_case_cap_min_cells=internal.get("init_ratio_case_cap_min_cells", 640),
            init_ratio_case_cap_random_state=internal.get(
                "init_ratio_case_cap_random_state",
                20260420,
            ),
            fix_point_factor=internal.get("fix_point_factor", 0.3),
            weak_ratio=internal.get("weak_ratio", 0.03),
            n_neighbors=config.n_neighbors,
            graph_method=internal.get("graph_method", "umap"),
            n_bg_permutations=internal.get("n_set", 0),
        ),
        sampling_options=SamplingOptions(
            n_cells=config.n_cells,
            n_subsamples=config.n_subsamples,
            weight_method=internal.get("weight_method", "softmax"),
    ...
    logger.event(
        ...
        "Running the default Step 2 refinement path.",
        audience="reviewer",
    ...
    sample_packs, step2_result = run_step2_from_step1(
        ...
        bridge_options=Step2BridgeOptions(
            sample_layer=internal.get("sample_layer", "central_norm"),
            runner=internal.get("step2_runner", "serial"),
            max_workers=internal.get("step2_max_workers", 2),
            iterations=internal.get("step2_iterations"),
            step2_options=Step2Options(
                n_pcs=internal.get("step2_n_pcs", 50),
                cell_k=config.step2_cell_k,
                max_iterations=internal.get("step2_max_iterations", 50),
                stable_rounds=internal.get("step2_stable_rounds", 4),
                score_mode=internal.get("step2_score_mode", "self_n_label"),
                threshold_k=internal.get("step2_threshold_k", 2.0),
                drop_limit=internal.get("step2_drop_limit", True),
                dtype=internal.get("step2_dtype", "float32"),
                force_connect=internal.get("step2_force_connect", True),
                _smooth_weight=internal.get("step2_smooth_weight", 0.15),
                _smooth_alpha=internal.get("step2_smooth_alpha", 0.12),
                _binary_delta_threshold=internal.get("step2_binary_delta_threshold", 0.1),
                _guide_compare_rounds=internal.get("_step2_guide_compare_rounds"),
                _relaxed_threshold_on_weak_pert=internal.get(
                    "_step2_relaxed_threshold_on_weak_pert",
                    True,
                ),
                _relaxed_threshold_latch_weak_pert=internal.get(
                    "_step2_relaxed_threshold_latch_weak_pert",
                    False,
                ),
                _relaxed_threshold_min_k=internal.get("_step2_relaxed_threshold_min_k"),
                _relaxed_threshold_scale=internal.get("_step2_relaxed_threshold_scale", 0.6),
                _legacy_post_stable_rounds=internal.get("_step2_legacy_post_stable_rounds", 3),
            ),
        ),
        logger=logger,
    )
    ...
    response_score_merge_enabled = bool(internal.get("_result_score_history_merge", False))
    ...
    result_ad = None
    if internal.get("_result_build", True):
        result_ad = build_result_anndata(
            ...
            metadata={
                ...
                "step2_threshold_k": internal.get("step2_threshold_k", 2.0),
                "post_step2_refine_k": internal.get("_result_refine_k", 3.0),
                "random_state": runtime.random_state,
            ...
            merge_top_n=internal.get("_result_merge_top_n", 3),
            merge_mode=(
                "all"
                if config.n_cells > 80
                else internal.get("_result_merge_mode", "step2_representative")
            ),
            graph_method=internal.get("_result_graph_method", "gauss"),
            compute_gene_pair=internal.get("_result_compute_gene_pair", False),
            graph_fs_mode=internal.get("_result_graph_fs_mode", "handoff"),
        )
